    ...
    def molecule_fragment_builder(self):
        ...
        if self.MOLECULE_FRAGMENT_BUILDER_cmds:
            ...
            for cmdi, cmd in enumerate(self.MOLECULE_FRAGMENT_BUILDER_cmds, 1):
                ...
                if params not in self.ion_defs_built.keys():
                    self.ion_defs_built[params] = {"positive": {}, "negative": {}}
                self.ion_defs_built[params]["positive"].update(copy.deepcopy(lipid))
                self.ion_defs_built[params]["negative"].update(copy.deepcopy(lipid))
